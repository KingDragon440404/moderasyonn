...
if(!["789115192838586399"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission(""))) 
...
if(!kullanici) return message.channel.send(new MessageEmbed().setDescription(`<a:hydratac:789369824249643009> ${message.author}, <a:hydratac:789369824249643009> **Bir kullanıcı etiketlemelisin.** `).setImage('https://media.discordapp.net/attachments/882701563804651625/882992473822097408/53-1.gif?width=374&height=210').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0x800d0d').setTimestamp()).then(x => x.delete({timeout: 5000000}));

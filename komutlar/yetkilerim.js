...
exports.run = async (client, msg, args) => {
  ...
  //yönetici
  if (msg.member.hasPermission("ADMINISTRATOR")) yönetici = `:white_check_mark:`
  ...
  //Sunucu yönet
  if (msg.member.hasPermission("MANAGE_GUILD")) sunucuyuyönet = `:white_check_mark:`
  if (!msg.member.hasPermission("MANAGE_GUILD")) sunucuyuyönet = `:x:`
  
  //Rolleri Yönett :D
  if (msg.member.hasPermission("MANAGE_ROLES")) rolleriyönet = `:white_check_mark:`
  if (!msg.member.hasPermission("MANAGE_ROLES")) rolleriyönet = `:x:`
  
  //Kanalları Yönet :D:D:SA:DASD:asD:sa
  if (msg.member.hasPermission("MANAGE_CHANNELS")) kanallarıyönet = `:white_check_mark:`
  if (!msg.member.hasPermission("MANAGE_CHANNELS")) kanallarıyönet = `:x:`
  
  //üyeleri at qwewqeqwe
  if (msg.member.hasPermission("KICK_MEMBERS")) üyeleriat = `:white_check_mark:`
  ...
  //davet o.
  if (msg.member.hasPermission("CREATE_INSTANT_INVITE")) davet = `:white_check_mark:`
  if (!msg.member.hasPermission("CREATE_INSTANT_INVITE")) davet = `:x:`
  
  //kullanıcı adı değiştir
  if (msg.member.hasPermission('CHANGE_NICKNAME')) kadeğiştir = `:white_check_mark:`
  if (!msg.member.hasPermission('CHANGE_NICKNAME')) kadeğiştir = `:x:`
  
  //ka yönet :)
  if (msg.member.hasPermission('MANAGE_NICKNAMES')) kayönet = `:white_check_mark:`
  ...
  if (msg.member.hasPermission('MANAGE_WEBHOOKS')) webhook = `:white_check_mark:`
  if (!msg.member.hasPermission('MANAGE_WEBHOOKS')) webhook = `:x:`
  
  //met ses oku gör işte len :D
  if (msg.member.hasPermission('VIEW_CHANNEL')) kanallarıgör = `:white_check_mark:`
  if (!msg.member.hasPermission('VIEW_CHANNEL')) kanallarıgör = `:x:`
  
  //mesaj gönderme looo
  if (msg.member.hasPermission('SEND_MESSAGES')) mesajgönder = `:white_check_mark:`
  ...
  //mesajları yönetme looo :ASD:Asd:ASd
   if (msg.member.hasPermission('MANAGE_MESSAGES')) mesajyönet = `:white_check_mark:`
  ...
  //GEÇMİŞİ GÖRÜMTÜLEEEEE :d
   if (msg.member.hasPermission('READ_MESSAGE_HISTORY')) geçmiş = `:white_check_mark:`
  if (!msg.member.hasPermission('READ_MESSAGE_HISTORY')) geçmiş = `:x:`
  
  //ever here felan filan
   if (msg.member.hasPermission('MENTION_EVERYONE')) everhere = `:white_check_mark:`
  if (!msg.member.hasPermission('MENTION_EVERYONE')) everhere = `:x:`
  
  //harici emojiiiiii
  if (msg.member.hasPermission('USE_EXTERNAL_EMOJIS')) hariciemoji = `:white_check_mark:`
  if (!msg.member.hasPermission('USE_EXTERNAL_EMOJIS')) hariciemoji = `:x:`
  
  //tepki ekle
   if (msg.member.hasPermission('ADD_REACTIONS')) tepkiekle = `:white_check_mark:`
  ...
  //sağırlaştır 
  if (msg.member.hasPermission('DEAFEN_MEMBERS')) sağırlaştır = `:white_check_mark:`
  ...
  //öncesi sonrası yok karşiiiim 
   if (msg.member.hasPermission('PRIORITY_SPEAKER')) önceliklikonuşmacı = `:white_check_mark:`
  if (!msg.member.hasPermission('PRIORITY_SPEAKER')) önceliklikonuşmacı = `:x:`
  const embed = new Discord.MessageEmbed()
  .setTitle('HYDRA ┃ Yetkilerim')
  .setColor('RED')
